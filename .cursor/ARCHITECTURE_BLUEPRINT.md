...
**般若计划**下的 **留痕**（Liuhen）：**Windows 采集（WinRecorder）+ 回放/直播（OmniPlayer）+ 仪表盘**；边载 **Android 采集**（`omnitrace_android`，**当前几乎不可用**，本期不进 Windows OmniPlayer）。关于页：情报和信息采集，以及现象世界模型运行日志；也可作自我侧写。数据默认本机 `OmniDatabase/`，不上传。公开仓库 MIT：`https://github.com/Fortda/liuhen`。
