...
> 只写笔记 / MCP / 线索板 / 时间轴 / 参数窗等「现在是什么」；改契约时同步改主蓝图那一句。
...
- **流式状态**：等待时显示「等待服务器响应… / 思考中… / MCP 回合…」。**有 MCP 时 tool 循环为非流式**（长文/思考会久），Rust 每 5s 心跳刷新状态并写入 `request_sent`/`first_byte`，禁止一直空白 TTFB；工具失败显示在卡片状态。**MCP 活动面板**：`notes-stream-status` 可带结构化 `activity`（`phase` / `round` / `thinking` / `tools[]` 含 name·target·status；思考多段用 `---` 拆），整板可折叠（运行中展开、结束后默认收起）。**可见动作词汇**：思考（独立块）+ 工具按名/显式桶归 **读取 / 编辑 / 命令行**（偶发无法归类时短标 **MCP**；旧 explore→read、edited→edit）；桶内保持调用序。**轮次渐进折叠**：当前运行轮最细（正在执行的思考/工具步展开）；上一轮只留概览短标签（可再展开）；更早轮整轮收起（一行 chevron，可手动展开）。新轮开始时前一轮降为概览、再前一轮收起。**结束后仍保留**（落盘 `mcp_activity`；旧卡可从协议日志重建）。展示为 Cursor 式无框折叠行，展开控件不是描边按钮。`data` 抽屉仍是原始协议日志。**SSE `error` 帧与空正文**一律标 `status=error` 并带回原文（禁止静默空成功）。
...
- **用户气泡**：超约 6 行（`1.55em × 6` ≈ 132px）默认折叠，可展开/收起；短消息不折。流式新发送同样默认折。Composer 工具栏 **色温**（`notes_color_temp.ts`）：击键间隔短偏暖红、长偏冷蓝；**退格变灰**（删掉的字标 `deleted`，可单独开关显示）；只作用于输入框与用户气泡（助手不变）；开关 `localStorage omnitrace.notes.colorTemp` / `omnitrace.notes.showBackspace`（退格缺省开），字形随卡片落盘 `user_glyphs`（含 deleted）。线索板工具栏同一套按钮与偏好，便签正文落盘 `glyphs`。
...
- UI：`notes_clue_board.ts`。便签右上六点球冠叫「把手」；左键拖把手挪便签，**拖便签过程中再按右键 = 平移视口**（由 `buttons` 位图驱动，不依赖单独的 RMB `pointerdown`——WebView2 在 LMB `setPointerCapture` 时常丢该事件；中键为后备；便签跟手补偿；松辅键停，无惯性；平移超阈值则不弹右键菜单）；空白处仍左键长按平移 / 右键菜单或框选；滚轮缩放保留；点文字编辑亦 `is-selected` 蓝光；Ctrl/Cmd+点多选便签；**Ctrl/Cmd+点亦可多选红线连线**（互斥于便签多选）；**多选便签时拖任一角缩放把手，按同 delta 同步缩放**（相对尺寸保留）。有子孙的便签左下 **chevron+计数** 可折叠/展开；折叠后隐藏子孙及连到隐藏节点的边。便签可选 `parentId` / `collapsed` / `kind`（`project`|`research`，同一平面轻标签，不是 2.5D 图层）。左列表竖标签无删除叉；标题右侧紧凑 **+** 新建板；瓷砖显示便签/线数与相对最近修改（`updated_at`，缺则 `created_at`，再缺显示「未知」）；悬停 `.omni-float` 出便签数、线数、创建/修改时刻与近期 history 快照（最多约 8 条，无则如实空）；右键「详情 / 多选 / 删除」（仅多于一块板时出删除，确认同前）。**Ctrl/Cmd+点**进多选、**Shift+点**连选、**Ctrl/Cmd+A**（焦点在侧栏列表）全选、**Esc** 退出多选；普通单击仍切换当前板。
- **图片**：工具栏「添加图片」、空白处右键、或把图片文件拖上画板。剪贴板有图且焦点不在文本框时 Ctrl/Cmd+V 可贴成新图片卡；**焦点在便签正文 textarea 时**，有图则贴到**该便签**（写 `image` 路径引用并显示预览，不另开卡），纯文字仍走字段粘贴；图文并存时优先贴图（不插入乱文字）。字节在 `notes/config/clue_images/<文件>`（内容哈希文件名）；节点字段 `image` 只存相对引用 `clue_images/…`，禁止 base64 进 JSON。图片卡与便签一样可拖、选、删、进复制粘贴。
- **复制粘贴**：右键「复制 / 粘贴」。Ctrl/Cmd+C 复制当前所选便签（含图片卡）。焦点在 input/textarea/contenteditable **且选中了字符**时，Ctrl/Cmd+C 交给浏览器复制那段文字，不复制便签；框有焦点但没有选中字符时仍复制便签。Ctrl/Cmd+V：便签正文框内有图→贴到该便签，无图→普通文字粘贴；不在文本框时粘贴便签，若剪贴板是图片则新建图片卡。粘贴生成新 id、相对原位略偏移（空白处右键粘贴则落到点击处），组内 `parentId` / `kind` / 两端都在所选集合内的连线会重映射；指向集合外的父级和连线丢掉。图片引用原样共享同一文件。
...
- **版本史**（Photoshop 式：一板一史）：每板 append-only `notes/config/clue_history/<board_id>.jsonl`（全量快照 + `actor=human|ai`）；仅记录该板内容变更（便签/连线/视口/标题/回退/初始种子）。**切换板 / `set_active` 不写历史**。回退=恢复目标 seq 快照并追加 `rollback` 行（不截断旧日志）。快照里的图片只存 `image` 路径引用，不含二进制；回退不删、不改 `clue_images/` 文件，删除便签也不删文件（旧快照仍可能引用）。工具栏「历史」面板与 MCP `clue_board_list_history` / `clue_board_rollback` 共用。
...
- 摘窗：`shell_open_notes_window` / `shell_close_notes_window` / `notes-dock-changed` 等（XOR 契约见主蓝图 §2 / §5.1）。
