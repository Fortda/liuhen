...
- UI：`notes_clue_board.ts`。便签右上六点球冠叫「把手」；左键拖把手挪便签，**拖便签过程中再按右键 = 平移视口**（应用内 capture 监听，textarea/图片上也可；便签跟手补偿；松右键停，无惯性；若已平移则不弹右键菜单）；空白处仍左键长按平移 / 右键菜单或框选；滚轮缩放保留；点文字编辑亦 `is-selected` 蓝光；Ctrl/Cmd+点多选便签；**Ctrl/Cmd+点亦可多选红线连线**（互斥于便签多选）；**多选便签时拖任一角缩放把手，按同 delta 同步缩放**（相对尺寸保留）。有子孙的便签左下 **chevron+计数** 可折叠/展开；折叠后隐藏子孙及连到隐藏节点的边。便签可选 `parentId` / `collapsed` / `kind`（`project`|`research`，同一平面轻标签，不是 2.5D 图层）。左列表竖标签无删除叉；标题右侧紧凑 **+** 新建板；右键「详情 / 多选 / 删除」（仅多于一块板时出删除，确认同前）。Ctrl/Cmd+点列表也可多选。
- **图片**：工具栏「添加图片」、空白处右键、或把图片文件拖上画板。剪贴板有图且焦点不在文本框时 Ctrl/Cmd+V 也可贴图。字节在 `notes/config/clue_images/<文件>`（内容哈希文件名）；节点字段 `image` 只存相对引用 `clue_images/…`，禁止 base64 进 JSON。图片卡与便签一样可拖、选、删、进复制粘贴。
- **复制粘贴**：右键「复制 / 粘贴」。Ctrl/Cmd+C 复制当前所选便签（含图片卡）。焦点在 input/textarea/contenteditable **且选中了字符**时，Ctrl/Cmd+C 交给浏览器复制那段文字，不复制便签；框有焦点但没有选中字符时仍复制便签。Ctrl/Cmd+V 在文本框内走普通文字粘贴；不在文本框时粘贴便签，若剪贴板是图片则贴图。粘贴生成新 id、相对原位略偏移（空白处右键粘贴则落到点击处），组内 `parentId` / `kind` / 两端都在所选集合内的连线会重映射；指向集合外的父级和连线丢掉。图片引用原样共享同一文件。
