...
**留痕**（英文 Liuhen）是辅助人尽可能记录和模拟世界的软件，也可作自我侧写。

这是作者作为外行、**100% 面向 AI 编程**做出来的项目。

它是般若计划下的一套情报和信息采集与现象世界模型运行日志维护、管理的终端；意在扩展感知在现象世界时空因果链网络上的深度与广度。
...
- **磁盘占用与日常调用覆盖率（识别/设计中）**：打算观察本机各盘占用、以及哪些区域常被访问、哪些是冷归档，好在清理时不盲删珍贵旧数据；空闲备用盘也能被看见。只在本机，不上传。现在还没有爬盘或热度采集成品。
...
- **线索板**：一张二维画布，用便签、图片和连线整理思路。可选中后复制粘贴（Ctrl+C / Ctrl+V）。
...
- **磁盘占用与日常调用覆盖率**（自我观测：卷空闲、目录抽样、可选后期访问热度；帮助区分热数据与冷归档；不自动删文件、不做云备份）
