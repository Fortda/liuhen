...
**软件定位**是一套情报和信息采集与现象世界模型运行日志维护、管理的终端；意在扩展感知在现象世界时空因果链网络上的深度与广度，我打算叫这个项目为般若计划，很帅
...
- 键鼠记录已经压缩过，按每天高强度用机约 8 小时算，一年大概不会超过 4 GB，本机高峰一天约 15 MB；窗口那些其它日志还没怎么压。
...
- **Android**（`omnitrace_android/`）：自行安装的手机试用版，**目前几乎没法用**。以后打算在本机局域网和电脑联动；现在还没有接到 Windows 应用里。虽然几乎没法用但也可以下载下来玩玩
...
| [docs/releasing.md](docs/releasing.md) | 怎么下载；维护者怎么打 setup.exe / zip |
